    ...
    return {
        ...
        controller:function($scope) {
            ...
            $scope.itemSelect = function(item) {
                ...
                $scope.nodeItem = selectGrandParent($scope.list,item);
                HierarchyNodeService.findParent($scope.nodeItem,null,item,selectParent);
            ...
            function selectGrandParent(list1,item)
            {
                
                var keepGoing = true;
                var returnDetails;
                var list = list1;
                if (angular.isDefined(list1.items) && list1.items.length > 0) 
                {
                    list = list1.items;
                }
                angular.forEach(list,function(value,key){
                    if (keepGoing ) 
                    {
                        ...
                        else if (angular.isDefined(value) && angular.isDefined(value.items) && value.items.length > 0) 
                        {
                            returnDetails =  selectGrandParent(value,item);
                            if (angular.isDefined(returnDetails)) 
                            {
                                keepGoing = false;
                            }
                        }
            ...
            function selectParent(parent) {
                ...
                $scope.nodeItem = selectGrandParent($scope.list,parent);

                HierarchyNodeService.findParent($scope.nodeItem,null,parent,selectParent)
            }       
